...
# Load the CSV file
file_path = f"{path}rankings_history.csv"
...
# Filter ranked fighters (0 to 15) from specified categories
...
df_ranked = df[(df["division"].isin(categories)) & (df["rank"].between(1, 15, inclusive="both")) & (df["date"].notna()) & (df["date"] >= date_start) & (df["date"] <= date_end)]
...
output_path_ranked = f"{path}ranked1-15_unique_fighters.csv"
