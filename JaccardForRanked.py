...
# Example: The “unique P4P fighters” list
P4Punique = pd.read_csv('C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/UFCDATA/pound-for-pound/ranked1-15_unique_fighters.csv')
unique_ranked = P4Punique['Fighter'].to_numpy()

p4p_set = {name.lower().strip() for name in unique_ranked}
#%%
# -------------------------------------------------------------------
...
def get_top_for_20_fighters(df: pd.DataFrame, max_fighters=363) -> pd.DataFrame:
    """
...
for key, df_sub in metrics_data_2013feb_2021mar.items():
    top_df = get_top_for_20_fighters(df_sub, max_fighters=363)  
    # ^ You mentioned 363 in your code, change as needed
...
for name, df_sub in metrics_data_2013feb_2021mar.items():
    sum_series = df_sub.sum(axis=1, skipna=True)
    topN = sum_series.nlargest(363)  # again, your code used 363
...
for name, df_sub in metrics_data_2013feb_2021mar.items():
    ...
    topN = per_point_series.nlargest(363)
    topN_df = pd.DataFrame({
        "Fighter": topN.index,
