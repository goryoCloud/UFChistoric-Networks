...
# ----------------------------------------------------------------------
# 2) DEFINE A HELPER FUNCTION TO FILTER COLUMNS FROM 2003-04-02 ONWARDS
# ----------------------------------------------------------------------

def filter_from_2003_04_02_onwards(df: pd.DataFrame) -> pd.DataFrame:
    """
    Given a DataFrame whose columns are date strings like 'YYYY-MM-DD HH:MM:SS',
    return a new DataFrame with only the columns from 2003-04-02 onwards.
    """
    # Convert columns to datetime
    col_dates = pd.to_datetime(df.columns, errors='coerce')
    # Create a boolean mask: True if date >= 2003-04-02
    mask = col_dates >= pd.to_datetime("2003-04-02")
...
# ----------------------------------------------------------------------
# 3) BUILD A NEW DICTIONARY "metrics_data_post2003" FOR FILTERED DATA
# ----------------------------------------------------------------------

metrics_data_post2003 = {}
for name, df_sub in metrics_data.items():
    df_filtered = filter_from_2003_04_02_onwards(df_sub)
    metrics_data_post2003[name] = df_filtered
...
results_dict_post2003 = {}
for key, df_sub in metrics_data_post2003.items():
    top_df = get_top_for_20_fighters(df_sub)
    results_dict_post2003[key] = top_df

# ----------------------------------------------------------------------
# 4B) SUM VALUES ACROSS COLUMNS FOR EACH FIGHTER, STORE IN sum_results_post2003
# ----------------------------------------------------------------------

sum_results_post2003 = {}
for name, df_sub in metrics_data_post2003.items():
    sum_series = df_sub.sum(axis=1, skipna=True)
    sum_df = pd.DataFrame({
        "Fighter": sum_series.index,
        "SumValue": sum_series.values
    })
    sum_results_post2003[name] = sum_df

# ----------------------------------------------------------------------
# 4C) GET TOP 20 FIGHTERS BY SUM (FILTERED DATA)
# ----------------------------------------------------------------------

top20_sums_dict_post2003 = {}
for name, df_sub in metrics_data_post2003.items():
    sum_series = df_sub.sum(axis=1, skipna=True)
    ...
    top20_sums_dict_post2003[name] = top20_df
...
for name, df_sub in metrics_data_post2003.items():
    ...
    pp_results_post2003[name] = pp_df

top20_pp_dict_post2003 = {}
for name, df_sub in metrics_data_post2003.items():
    sum_series = df_sub.sum(axis=1, skipna=True)
    count_series = df_sub.count(axis=1)
    ...
    top20_pp_dict_post2003[name] = top20_df

# ----------------------------------------------------------------------
# DONE: At this point, you have new dictionaries:
#   results_dict_post2003     : top 20 fighters (one row per fighter, by highest value)
#   sum_results_post2003      : sum across columns for each fighter
#   top20_sums_dict_post2003  : top 20 fighters by sum
#   pp_results_post2003       : per-point values for each fighter
#   top20_pp_dict_post2003    : top 20 fighters by per-point
# all of which are restricted to columns (dates) >= 2003-04-02.
# ----------------------------------------------------------------------
