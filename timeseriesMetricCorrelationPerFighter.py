...
#%%
pathDataframes = 'C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/UFCDATA/dataframes3/degreeEvol.csv'
...
for fighter in fighterNames:
    ...
    try:
        ...
        pearson = pearsonCoeff(merged_data['rank'], merged_data[fighter])[0]
...
ax1.plot(deg['date'], deg[fighter], lw = 2, color = 'magenta')
ax1.set_xlabel('date')
ax1.set_ylabel(r'$\lambda$', color='magenta')
ax1.tick_params('y', colors='magenta')
ax1.set_ylim(min(deg[fighter]))
#ax1.set_xlim(len(deg['date']))

ax2 = ax1.twinx()
ax2.plot(google['date'], google['rank'], lw = 1, color = 'black')
ax2.set_ylabel('Pound-for-pound rank')
ax2.set_ylim(0)

plt.title('Conor McGregor - Eigenvector centrality vs. Pound-for-pound rank')
ax1.annotate(f'Pearson: {pearson:.3f}', xy=(0.17, 0.85), xycoords='figure fraction', fontsize=12)

plt.show()

#%%

plt.plot(google['date'], google['rank'], lw = 2, color = 'blue')
plt.title('Conor McGregor')
plt.ylabel('Google interest')
plt.xlabel('date')
plt.ylim(0)
#import csv
#output_csv_path = 'C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/UFCDATA/timeseriesCorrelations/eigenEvolCorrelation.csv'
