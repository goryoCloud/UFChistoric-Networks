...
#    plt.show()
    ...
    start_date += freq
    end_date += freq
#%% SAVING ROUTINES - METRICS
metricsDF = pd.DataFrame({'deg': deg, 'density': den, 'clustering': clust, 'Ginni': gin, 'pathLenght': pathLenght, 'betweenessCentrality': betweeness, 'eigenvectorCentrality': eigenvector, 'PPVsales': PPVsales})
metricsDF.to_csv(f'C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/plotsMetrics/metrics_{category}.csv', index=False)
#%%
...
ax2 = ax1.twinx()
ax2.plot(PPVsales,'-k', ms = 1, lw = 1.5, alpha = 0.7)
ax2.set_ylabel('Number of PPV sales', color='k')
ax2.tick_params('y', colors='k')
...
ax1.set_xlabel('window index')
ax1.set_ylabel(r'$b$', color='darkgreen')
...
lstStart = first_non_zero_index(deg)

if lstStart <= PPVstart:
    startingIndex = 67
else:
    startingIndex = lstStart

PPVnonZero = PPVsales[startingIndex:286]

#-----------------------------------------------------
clustPPV =  clust[startingIndex:286]
corrClust = pearsonr(PPVnonZero, clustPPV)

degPPV = deg[startingIndex:286]
corrDeg = pearsonr(PPVnonZero, degPPV)

denPPV = den[startingIndex:286]
corrDen = pearsonr(PPVnonZero, denPPV)

ginPPV = gin[startingIndex:286]
corrGin = pearsonr(PPVnonZero, ginPPV)

nFightsPPV = numFights[startingIndex:286]
corrNFights = pearsonr(PPVnonZero, nFightsPPV)

nFightersPPV = numFighters[startingIndex:286]
corrNFighters = pearsonr(PPVnonZero, nFightersPPV)

lenghtPPV = pathLenght[startingIndex:286]
corrPath = pearsonr(PPVnonZero, lenghtPPV)

betPPV = betweeness[startingIndex:286]
corrBet = pearsonr(PPVnonZero, betPPV)

eigenvectorPPV = eigenvector[startingIndex:286]
...
#plt.xlabel('metric')
#%%
correlationDF = pd.DataFrame({'corrClust': corrClust, 'corrDeg': corrDeg, 'corrDen': corrDen, 'corrGin': corrGin, 'corrPath': corrPath, 'corrBet': corrBet, 'corrEigen': corrEigen})
correlationDF.to_csv(f'C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/plotsMetrics/correlations_{category}.csv', index=False)