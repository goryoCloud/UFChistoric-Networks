...
P4Punique = pd.read_csv('C:/Users/max_s/Desktop/MAGISTER-MAX/NOTES/UFC/UFCDATA/pound-for-pound/p4p_unique_fighters.csv')
...
def get_top_for_20_fighters(df: pd.DataFrame, max_fighters=len(P4Punique)) -> pd.DataFrame:
    """
...
for key, df_sub in metrics_data_2013feb_2021mar.items():
    top_df = get_top_for_20_fighters(df_sub, max_fighters=len(P4Punique))  
    # ^ You mentioned 40 in your code, change as needed
...
for name, df_sub in metrics_data_2013feb_2021mar.items():
    sum_series = df_sub.sum(axis=1, skipna=True)
    topN = sum_series.nlargest(len(P4Punique))  # again, your code used 40
...
for name, df_sub in metrics_data_2013feb_2021mar.items():
    ...
    topN = per_point_series.nlargest(len(P4Punique))
    topN_df = pd.DataFrame({
        "Fighter": topN.index,
